...
class Semaphore {
    ...
    wait(process) {
        if (this.value > 0) {
            ...
            if (window.app && window.app.logger) {
                window.app.logger.log(process.name, `Acquired Semaphore`, `S${this.id} (value: ${this.value})`, 'system');
            }
            ...
            if (window.app && window.app.logger) {
                window.app.logger.log(process.name, `Waiting on Semaphore`, `S${this.id} — queue: [${this.waitQueue.map(id => {
    ...
    signal(process) {
        ...
        if (window.app && window.app.logger) {
            window.app.logger.log(process.name, `Released Semaphore`, `S${this.id} (value: ${this.value})`, 'system');
        }
        ...
        if (this.waitQueue.length > 0) {
            ...
            if (nextProcess) {
                nextProcess.unblock();
                if (window.app && window.app.logger) {
                    window.app.logger.log(nextProcess.name, `Acquired Semaphore (woken)`, `S${this.id}`, 'system');
                }
            }
        } else {
            this.heldBy = null;
