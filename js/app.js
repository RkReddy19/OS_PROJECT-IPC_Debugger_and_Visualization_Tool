...
class App {
  ...
  bindEvents() {
    ...
    document
      ...
      ?.addEventListener('click', () => this.logger.exportReport());
  ...
  loadScenario(scenarioName) {
    ...
    this.logger.log(
      ...
      `${this.scenarioSteps.length} steps`,
      'system'
    );
  }
  ...
  setupRaceConditionScenario() {
    ...
    this.scenarioSteps.push((app) => {
      ...
      app.logger.log(
        'P3',
        `Read counter = ${val.value}`,
        'Expected: 2, race condition may have caused data loss',
        'receive'
      );
      app.timeline.recordEvent(p3.id, 'receive');
    });
    ...
    this.scenarioSteps.push((app) => {
      ...
      app.logger.log(
        'P3',
        `Safe counter = ${val.value}`,
        'Correctly = 2, semaphore prevented race',
        'receive'
      );
    });
  ...
  renderVisualization() {
    ...
    const vizConnections = this.connections.map((conn) => ({
      ...
      isActive:
        conn.ref.isActive ||
        (conn.ref.queue && conn.ref.queue.length > 0) ||
