  ...
  send(fromProcess, data, priority = 0) {
    if (this.queue.length >= this.maxCapacity) {
      ...
      if (window.app && window.app.logger) {
        window.app.logger.log(
          fromProcess.name,
          'Queue OVERFLOW',
          `Queue ${this.id} full [${this.queue.length}/${this.maxCapacity}]`,
          'blocked'
        );
      }
    ...
    if (window.app && window.app.logger) {
      window.app.logger.log(
        fromProcess.name,
        'Sent to Queue',
        `"${data}" [${this.queue.length}/${this.maxCapacity}] prio:${priority}`,
        'send'
      );
    }
  ...
  receive(toProcess) {
    ...
    if (window.app && window.app.logger) {
      window.app.logger.log(
        toProcess.name,
        'Received from Queue',
        `"${message.data}" from ${message.from} [${this.queue.length}/${this.maxCapacity}]`,
        'receive'
      );
    }
